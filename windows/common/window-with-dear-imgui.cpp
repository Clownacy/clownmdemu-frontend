...
void WindowWithDearImGui::ReloadFonts(const unsigned int font_size)
{
	...
	*std::format_to_n(font_cfg.Name, std::size(font_cfg.Name) - 1, "Noto Sans Regular, {}px", font_size).out = '\0';
	...
	*std::format_to_n(font_cfg.Name, std::size(font_cfg.Name) - 1, "Inconsolata Regular, {}px", font_size).out = '\0';
...
WindowWithDearImGui::WindowWithDearImGui(const char* const window_title, const int window_width, const int window_height, const bool resizeable)
	: Window(window_title, window_width, window_height, resizeable)
