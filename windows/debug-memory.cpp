...
template<>
std::string DebugMemory::PrintLineInternal(const cc_u8l* const buffer, const std::size_t buffer_length_digits, const int index)
{
	const int offset = index * 0x10;
	const cc_u8l* const bytes = &buffer[offset];

	return fmt::format("{:0{}X}: {:02X} {:02X} {:02X} {:02X}"
	                           " {:02X} {:02X} {:02X} {:02X}"
	                           " {:02X} {:02X} {:02X} {:02X}"
...
template<>
std::string DebugMemory::PrintLineInternal(const cc_u16l* const buffer, const std::size_t buffer_length_digits, const int index)
{
	const cc_u16l* const words = &buffer[index * 8];

	return fmt::format("{:0{}X}: {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X}", index * 0x10, buffer_length_digits,
		words[0], words[1], words[2], words[3], words[4], words[5], words[6], words[7]);
}
