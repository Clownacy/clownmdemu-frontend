...
static void SaveConfiguration()
{
	...
	SDL::IOStream file = SDL::RWFromFile(GetConfigurationFilePath(), "w");
	...
	#define PRINT_BOOLEAN_OPTION(FILE, NAME, VARIABLE) \
		...
		if (!file_utilities.last_file_dialog_directory.empty())
		{
			...
			SDL_WriteIO(file, last_file_dialog_directory.data(), last_file_dialog_directory.size());
			PRINT_STRING(file, "\n");
		}
...
static void HandleMainWindowEvent(const SDL_Event &event)
{
	...
	switch (event.type)
	{
			...
			switch (event.gbutton.button)
			{
				case SDL_GAMEPAD_BUTTON_LEFT_SHOULDER:
					...
					if (quick_save_exists)
					{
						...
						SDL_RumbleGamepad(SDL_GetGamepadFromID(event.gbutton.which), 0xFFFF / 2, 0, 1000 / 8);
					}
				...
				case SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER:
					...
					SDL_RumbleGamepad(SDL_GetGamepadFromID(event.gbutton.which), 0xFFFF * 3 / 4, 0, 1000 / 8);
					break;
			}
		...
		case SDL_EVENT_DROP_FILE:
			drag_and_drop_filename = event.drop.data;
			break;
